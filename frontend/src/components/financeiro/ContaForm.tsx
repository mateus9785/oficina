    ...
    <Modal isOpen={isOpen} onClose={onClose} title={conta ? 'Editar Conta' : 'Nova Conta'} size="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <Select label="Status" value={status} onChange={(e) => setStatus(e.target.value as StatusConta)} options={[{ value: 'pendente', label: 'Pendente' }, { value: 'pago', label: 'Pago' }, { value: 'atrasado', label: 'Atrasado' }]} />
        </div>
