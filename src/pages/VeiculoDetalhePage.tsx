...
import { formatKm, formatPlaca, formatDate, formatCurrency } from '../lib/formatters';
...
export function VeiculoDetalhePage() {
  ...
  const { buscarVeiculo, editarVeiculo, removerVeiculo } = useVeiculoStore();
  const { buscarCliente } = useClienteStore();
  const { ordensDoVeiculo } = useOrdemServicoStore();
        ...
        <Card className="p-6">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{formatKm(veiculo.km)}</span>
            </div>
          </div>
