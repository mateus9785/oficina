...
export async function getNotificacoes(req: Request, res: Response): Promise<void> {
  ...
  const [estoqueBaixoRows] = await pool.execute(
    `SELECT id, nome, marca, quantidade, estoque_minimo
     FROM pecas
     WHERE usuario_id = ? AND estoque_minimo > 0 AND quantidade <= estoque_minimo
